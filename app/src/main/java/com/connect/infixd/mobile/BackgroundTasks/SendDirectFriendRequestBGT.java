...
public class SendDirectFriendRequestBGT extends AsyncTask<String, Void, IntroNotifyResponse> {
    ...
    public SendDirectFriendRequestBGT(Context ctx)
    {
        this.ctx = ctx;
    ...
    protected IntroNotifyResponse doInBackground(String... params) {
        ...
        senderId = params[0];
        targetMobileNumber = params[1];

        InfixdClient clientApi = new InfixdClient();
        IntroNotifyResponse introNotifyResponse = clientApi.sendDirectFriendRequestByNumberNotification(senderId,targetMobileNumber);
